...
def analyse_polarity_text(text: str, geo: object, usr: object, coords) -> Tweet:
    ...
    if geo:
        ...
        geo.pop("_api")
        geo["bounding_box"] = (
            geo["bounding_box"].__dict__ if geo["bounding_box"] is not None else None
        )
        geo["bounding_box"].pop("_api")

    if analysis.sentiment.polarity > 0:
        logger.debug(
            "Tweet: " + text + ", Polarity: Positive" + ", Country: " + str(geo),
            str(coords),
        )
        return Tweet(polarity=Polarity.POSITIVE, text=text, country=geo, coords=coords)
    elif analysis.sentiment.polarity == 0:
        logger.debug(
            "Tweet: " + text + ", Polarity: Neutral" + ", Country: " + str(geo),
            str(coords),
        )
        return Tweet(polarity=Polarity.NEUTRAL, text=text, country=geo, coords=coords)
    else:
        logger.debug(
            "Tweet: " + text + ", Polarity: Negative" + ", Country: " + str(geo),
            str(coords),
        )
